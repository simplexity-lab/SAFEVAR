...
class NoCrashEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        # self.config = route_indexer.next()
    ...
    def _load_and_run_scenario(self, args, config, route, weather_idx, traffic_idx, physics):
        """
        ...
        try:
            ...
            #no finish mjw
            # get distance/ speed
            distance = self.manager.get_nocrash_objective_data()
            TIT, TET, acc = self.manager.get_nocrash_analyze_data()
            print("distance:", distance, " TIT:", TIT, " TET:", TET, " acc:", acc)
    ...
    def get_entry_load_and_run(self, args):
        result = self._load_and_run_scenario(args, (19, 28), 8, 0)
        return result


    def run(self, args, phy0):
        """
        ...
        # print(config)
        # # run
        # self._load_and_run_scenario(args, config, (19, 28), 8, 0, phy0)

        # route_indexer.save_state(args.checkpoint)

        # mjw havent done
        config = self.route_indexer.next_mmfn()
        result, TET, TIT, acc = self._load_and_run_scenario(args, config, (19, 28), 6, 0, phy0)
        del config
    ...
    #
    #     # mjw havent done
    #     result, TET, TIT, acc = self._load_and_run_scenario(self.args, config, (19, 28), 8, 0, phy0)
    #
    #     # save global statistics
    #     print("\033[1m> Registering the global statistics\033[0m")
    #     # global_stats_record = self.statistics_manager.compute_global_statistics(route_indexer.total)
